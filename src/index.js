...
function initApp() {
  ...
  document
    .querySelector("#location-submit-button")
    .addEventListener("click", getSearchString);
...
function locationAddressSearch(query) {
  fetch(
    `https://maps.googleapis.com/maps/api/geocode/json?address=${query}&key=AIzaSyBfDBAUx0xCfMvsNyNHPea7yR3Fj-8ZLRU`
  )
    .then(checkStatus)
    .then(res => res.json())
    .then(function(data) {
